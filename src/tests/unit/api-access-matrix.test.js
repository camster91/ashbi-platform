...
const INTENTIONALLY_PUBLIC_ROUTES = {
  ...
  'GET /api/portal/review/:token': { category: 'capability token', reason: 'Media review share link (docs/media-review.md): SHA-256-hashed 256-bit token, expiry and revocation checked; returns only that session, its annotations and decisions. Rate limited per IP.' },
  'GET /api/portal/review/:token/file': { category: 'capability token', reason: 'Streams only the file of the session the review share link names; quarantined, unreviewable and scan-blocked files are withheld. Rate limited per IP.' },
  'POST /api/portal/review/:token/annotations': { category: 'capability token', reason: 'Named-guest comment on the session the review share link names; plain text, bounded, rate limited per IP.' },
  'POST /api/portal/review/:token/decisions': { category: 'capability token', reason: 'Approval decision via review share link, only when the link was created with allowDecision; append-only and audited. Rate limited per IP.' },
