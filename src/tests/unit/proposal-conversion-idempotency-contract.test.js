...
const invoiceService = readFileSync('src/services/proposalInvoice.service.js', 'utf8');
...
test('proposal invoice conversion returns the existing record and recovers concurrent uniqueness races', () => {
  assert.ok(invoiceService.match(/invoice\.findUnique\([\s\S]*?proposalId: proposal\.id/g)?.length >= 2);
  assert.match(invoiceService, /if \(existing\) return existing/);
  assert.match(invoiceService, /error\?\.code !== 'P2002'/);
  assert.match(invoiceService, /if \(concurrentInvoice\) return concurrentInvoice/);
});
