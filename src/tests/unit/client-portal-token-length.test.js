...
test('magic-link claims carry identifiers only, so any contact fits the redemption bound', async () => {
  // Largest name/email the contact schemas allow.
  ...
  await app.close();
  assert.ok(token.length < 500, `identifier-only token stays small (${token.length})`);
  assert.equal(clientPortalTokenRedeemSchema.safeParse({ token }).success, true);
});
