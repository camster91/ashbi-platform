...
export default async function bookkeepingRoutes(fastify) {
  ...
  fastify.get('/transactions', {
    ...
    const expenses = await request.prisma.expense.findMany({
      where: expenseFilter,
      include: { client: { select: { id: true, name: true } } },
      orderBy: { date: 'desc' }
    });
  ...
  fastify.get('/summary', {
    ...
    const invoiceStats = await request.prisma.invoice.aggregate({
      where: invoiceFilter,
      _sum: { total: true },
      _count: true
    });
    ...
    const paymentStats = await request.prisma.invoicePayment.aggregate({
      where: paidFilter,
      _sum: { amount: true },
      _count: true
    });
    ...
    const expenseStats = await request.prisma.expense.aggregate({
      where: expenseFilter,
      _sum: { amount: true },
      _count: true
    });
  ...
  fastify.get('/balance', {
    ...
    const outstandingInvoices = await request.prisma.invoice.findMany({
