...
export default async function invoiceRoutes(fastify) {
  ...
  // ─── POST /recurring — generate recurring invoice from retainer plan ─────────
  fastify.post('/recurring', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    const { clientId, retainerPlanId } = request.body || {};
    if (!clientId && !retainerPlanId) {
      return reply.status(400).send({ error: 'clientId or retainerPlanId required' });
    }

    // Find the retainer plan
    const where = retainerPlanId ? { id: retainerPlanId } : { clientId };
    const plan = await fastify.prisma.retainerPlan.findFirst({
      where,
      include: { client: { select: { id: true, name: true } } }
    });
    if (!plan) return reply.status(404).send({ error: 'Retainer plan not found' });
    if (plan.retainerStatus === 'CANCELLED') {
      return reply.status(400).send({ error: 'Retainer plan is cancelled' });
    }

    // Determine billing amount from tier
    const tierPricing = { '999': 999, '1999': 1999, '3999': 3999 };
    const monthlyAmount = plan.monthlyAmountCad || plan.monthlyAmountUsd || tierPricing[plan.tier] || 0;

    if (monthlyAmount === 0) {
      return reply.status(400).send({ error: 'Could not determine billing amount for plan' });
    }

    const currency = plan.currency || 'CAD';
    const now = new Date();
    const dueDate = plan.nextBillingDate
      ? new Date(plan.nextBillingDate)
      : new Date(now.getFullYear(), now.getMonth() + 1, plan.billingDay || 1);

    const subtotal = parseFloat(monthlyAmount.toFixed(2));
    ...
    const invoice = await fastify.prisma.invoice.create({
      data: {
        ...
        status: 'SENT',
        title: `${plan.tier === '999' ? 'Retainer' : plan.tier === '1999' ? 'Growth' : 'Scale'} Plan — ${plan.client.name}`,
        dueDate,
        issueDate: now,
        subtotal,
        taxRate,
        ...
        recurringNextDate: new Date(dueDate.getFullYear(), dueDate.getMonth() + 1, plan.billingDay || 1),
        notes: plan.serviceDescription || `Monthly retainer services — ${plan.hoursPerMonth} hours`,
        sentAt: now,
        lineItems: {
          create: [{
            description: `Monthly retainer services — ${plan.hoursPerMonth} hours (${plan.tier} tier)`,
            itemType: 'LABOR',
            quantity: 1,
            unitPrice: subtotal,
            total: subtotal,
    ...
    // Update plan's nextBillingDate
    await fastify.prisma.retainerPlan.update({
      where: { id: plan.id },
      data: {
        nextBillingDate: invoice.recurringNextDate,
        hoursUsed: 0, // Reset for new cycle
      }
    });
