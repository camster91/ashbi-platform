...
describe('critical portal and shell control semantics', () => {
  ...
  it('names chat fields, send actions, delete actions, and connection status', () => {
    ...
    // Labels live once in shared PortalChatComposer; two call sites mount it.
    expect(portal).toContain('aria-label="Message to project team"');
    expect(portal).toContain('aria-label="Send message"');
    expect(portal.match(/<PortalChatComposer/g)).toHaveLength(2);
