...
| Slack: internal/client conversation | Chat, inbox, realtime notifications, AI chat, OAuth v2/manual encrypted workspace setup, tenant channel mapping, signed inbound bridge, confirmed outbound mapped post | Code-present; production/provider validation pending | Deploy the Slack migrations and configure `SLACK_SIGNING_SECRET`, `SLACK_CLIENT_ID`, and `SLACK_CLIENT_SECRET`; install the Slack app in a sandbox workspace through OAuth or the admin recovery flow, map a project channel, and prove URL verification, valid delivery, invalid-signature rejection, stale replay rejection, duplicate delivery, confirmed outbound post, disconnect, retention, and notification preferences. Thread/channel parity and outbound retry/recovery remain open design work. |
...
| ChatGPT/AI control plane | Tenant-scoped OpenAI-compatible context bridge and confirmed internal task action | Read-only bridge plus code-present `create_task` action; broader action bridge not yet enabled | Connector/authentication test, provider health, prompt-injection review, broader allowlisted tools, and authenticated dogfood |
