...
| Slack: internal/client conversation | Chat, inbox, realtime notifications, AI chat, encrypted Slack workspace setup, tenant channel mapping, signed inbound event bridge | Code-present; production/provider validation pending | Deploy the Slack migrations and configure `SLACK_SIGNING_SECRET`; install the Slack app in a sandbox workspace through the admin flow, map a project channel, and prove URL verification, valid delivery, invalid-signature rejection, stale replay rejection, duplicate delivery, disconnect, retention, and notification preferences. Thread/channel parity and outbound delivery/retry remain open design work. |
