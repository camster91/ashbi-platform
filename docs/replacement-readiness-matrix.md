...
| Notion: project wiki and working documents | Project docs/notes and Docs navigation | Code-present; controlled Markdown-export import code-present; parity incomplete | Run `scripts/import-notion-markdown.js --organization-id <id> --project-id <id> --input-dir <Notion-export> --summary-file <new-report.json>` as a dry run, review counts/errors, retain a backup, then use `--confirm`; nested-page/template/mention workflow, import sample, real-tenant editing and recovery evidence remain required |
