...
| Slack: internal/client conversation | Chat, inbox, realtime notifications, AI chat, OAuth v2/manual encrypted workspace setup, tenant channel mapping, signed inbound event bridge | Code-present; production/provider validation pending | Deploy the Slack migrations and configure `SLACK_SIGNING_SECRET`, `SLACK_CLIENT_ID`, and `SLACK_CLIENT_SECRET`; install the Slack app in a sandbox workspace through OAuth or the admin recovery flow, map a project channel, and prove URL verification, valid delivery, invalid-signature rejection, stale replay rejection, duplicate delivery, disconnect, retention, and notification preferences. Thread/channel parity and outbound delivery/retry remain open design work. |
