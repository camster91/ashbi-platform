...
| --- | --- | --- |
| Slack | Mapped-channel inbound events to project chat with durable reply-thread reconciliation; installation/mapping administration; confirmed mapped outbound post or reply to an explicitly selected mapped-project thread; unknown outcomes are never automatically resent | Broad history search, retention policy, and sandbox approval; see [Slack outbound recovery policy](slack-outbound-recovery-policy.md) |
...
| Google Calendar | Ashbi-authoritative outbound creation/update for the connecting user's primary calendar; an event has one in-progress sync claim to prevent concurrent duplicate creation; a failed update with a known Google event ID can be explicitly retried without creating another event | Calendar selection, inbound or two-way reconciliation, recurring-event conflict policy, deletes, first-create failure reconciliation, and sandbox approval |
| ChatGPT/Codex | Read-only bounded tenant context (projects, tasks, clients, recent project conversations, and retainer utilization) plus confirmed internal task and project-calendar-event creation and mapped Slack post/reply | Other write tools, provider credentials, prompt-injection review, and authenticated dogfood evidence |
