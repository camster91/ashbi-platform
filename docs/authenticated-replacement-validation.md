...
| Delivery task and report | Staff creates exactly one project task under the contract-owned project, completes it with a durable completion time, and generates exactly one non-empty client report after both task completion and Stripe settlement reconciliation. The report remains linked to the same client, and replay does not create a second task or report. |
| Invoice/payment | For a manual draft, staff selects an authorized client and optional client-owned project, explicitly chooses CAD or USD, enters and attests to the reviewed tax type/rate, and creates one internal draft. Replay the same creation request and a simultaneous retry; both must return that same draft. For an approved proposal, confirm conversion creates or reuses one internal draft in the exact verified proposal currency and reviewed tax decision. Neither draft path may send email, create a payment link, or charge anything. After separate draft review, follow the [invoice email sandbox runbook](invoice-email-sandbox-validation.md): prove initial/resend request replay does not send twice, provider acceptance is not called inbox delivery, confirmed rejection is retryable, unknown outcome blocks retry pending reconciliation, and staff history retains every attempt. Then follow the [Stripe sandbox runbook](stripe-sandbox-validation.md): prove the persisted public token is the Checkout redirect token; exact replay returns one active session; expiry advances the attempt and permits one replacement; revoke, rotate, and void shield access and expire the exact active session before finalizing; completed/unknown invalidation races create durable reconciliation evidence; unpaid completion remains pending; delayed success pays once; delayed failure clears only the active attempt; signed successful/partial/failed refund events retrieve current provider state, retain idempotent append-only evidence, and display exact currency-labelled outcomes; stale-session, amount, currency, invoice-number, invalid-signature, over-refund, and voided-invoice events fail closed; administrator-triggered settlement reconciliation preserves payment versus balance currency, proves balance gross minus fee equals net, is idempotent, distinguishes pending/unknown evidence from zero, and refuses changed provider identity; and the final invoice/payment/refund/charge-settlement ledgers match Stripe without manual correction. |
...
Pass condition: the full proposal-to-payment, delivery-task, and client-report
chain reconciles across Ashbi, Stripe, email, and the client portal with no
