...
| WordPress bridge/fleet | Removed 2026-09-24 | Routes, jobs and UI deleted; `wp_*` tables retained unread pending a data-retention decision |
...
| Slack history migration | Controlled workspace-export importer, durable per-message reconciliation, and run rollback code-present; target migration evidence pending | The importer maps channels to projects and Slack members to Ashbi users by email, reports changed-source conflicts, unmapped channels/users, skipped bot/system messages, and unsupported files without overwriting them, and can roll back a run by id; follow [slack-export-migration.md](slack-export-migration.md) and validate a real workspace export before replacement claims ([#414](https://github.com/camster91/ashbi-platform/issues/414)) |
...
| Organization BYOK AI provider, budgets and kill switches | Code-present (slice 1 of [#413](https://github.com/camster91/ashbi-platform/issues/413)); not provider or target verified | [ai-byok.md](ai-byok.md): encrypted OpenAI-compatible key per organization, validation before storage, month-to-date metering against a budget, organization and deployment kill switches, audited admin actions. Policy numbers are proposals |
| Governed AI tool registry, approval queue and receipts | Code-present (slice 2 of [#413](https://github.com/camster91/ashbi-platform/issues/413)); not target verified | [ai-tool-registry.md](ai-tool-registry.md): closed tool catalogue with default-deny for external/irreversible tools, scope-checked execution, human approval with step-up, immutable receipts. [ai-evaluation.md](ai-evaluation.md): adversarial suite, beta requires all passing. No assistant route drives tool sessions yet; policy choices are proposals |
