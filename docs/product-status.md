...
| Tenant sales pipeline | Code-present and tenant-isolated locally; target evidence pending | Pipeline routes now use only the authenticated request scope; stage reads and mutations are organization-bound; deal creation requires a tenant-owned client and stage; the Hub/API `name` and `value` contract is aligned; empty stage configuration fails closed; and the unsupported AI close-score guess has been removed. Target stages/data, authenticated browser behavior, responsive/accessibility evidence, and the deliberate inquiry-to-deal connection remain pending. |
