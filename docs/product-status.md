...
| --- | --- | --- |
| Ashbi.ca inquiry capture and qualification | Code-present locally; not deployed or connected | Strict consent/attribution schema, explicit tenant and owner configuration, origin/privacy checks, idempotent tenant-owned lead/event creation, owner notification, a staff `/inquiries` review UI, auditable human qualification, and concurrency-safe inquiry-to-client conversion are present. Conversion reuses a tenant client on a case-insensitive contact-email match and sends no prospect message. The committed migration is unapplied; production tenant/owner/privacy values remain blank; Ashbi.ca is not connected; authenticated browser evidence, retention/deletion approval, and a controlled target test lead remain pending. |
| Tenant sales pipeline | Code-present and tenant-isolated locally; target evidence pending | Pipeline routes use only the authenticated request scope; deal creation requires a tenant-owned client and stage; and the Hub/API `name` and `value` contract is aligned. Staff can deliberately add and edit stages, while duplicate names and blank/invalid values are rejected. Empty-stage deletion requires confirmation; populated-stage deletion requires an explicit verified destination and confirmation. Empty configuration fails closed. The unsupported AI close-score guess is removed, and missing lifecycle evidence is no longer displayed as `0%` conversion. Target stages/data, authenticated browser behavior, responsive/accessibility evidence, true event-derived conversion metrics, ordering controls, and the deliberate inquiry-to-deal connection remain pending. |
