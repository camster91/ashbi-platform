...
Project identity approval is a separate gate from active-project disposition. Prepare and verify the native project-link packet from the exact native review as documented in [notion-markdown-migration.md](notion-markdown-migration.md). Its low-, medium-, and high-risk tiers cannot authorize project closure, task movement, owner assignment, billing changes, data migration, or Bonsai cutover. Generate the checksum-bound owner review brief before requesting a decision: the current brief identifies all 24 candidates as approval-ready—12 exact-title, five approved task-backed, and seven suggestions strengthened by sanitized live Notion/Bonsai project, task, and invoice-binding evidence. A recommendation is not an approval, and invoice history is not settlement evidence. Every decision still requires its explicit candidate ID; no decision applies a link or changes an external system.
...
The current 2026-08-28 UTC status is source-generation aligned but not migration- or retirement-ready. It contains nine blocking findings and has SHA-256 `801ad1f41bcc3798a5cf332244f90d30bdf6325ea5ea658e7d163466fb90324f`. All five mapping candidates, all 20 task identities, and all 20 source-backed owner candidates are recorded—19 Cameron and one Bianca—without applying mappings, links, or assignments. The corrected version 2 task-disposition layer now accounts for all 88 source tasks instead of only the 48 unmatched/malformed records. Its checksum-bound brief makes all 88 approval-ready: 40 source records resolved by the 20 approved identities, 43 valid unlinked tasks recommended for Hub migration, and five malformed/projectless Bonsai tasks recommended for repair plus recapture. All 88 decisions remain pending. A refreshed project-disposition brief classifies all 238 project dispositions: 190 are approval-ready, 48 source records remain blocked behind the 24 pending project links, and zero duplicate-title groups remain in manual review. The approval-ready set is 180 source-only Hub migrations, seven duplicate groups retained as distinct, and three same-client duplicate groups manually mapped with all source IDs preserved. Sanitized live evidence distinguishes the two Business Cards engagements and identifies Brand Discovery, Monthly SEO / Marketing, and Sola Identity as three same-logical-project groups; invoice existence and dates support identity only and are not payment or settlement evidence. The project-disposition decisions themselves all remain pending; all 65 active-project operating outcomes remain pending with zero closure authorization; and all 117 financial exceptions remain pending. Title, source status, source evidence, and a recommendation are not approval. Earlier live-status, task-disposition, and project-disposition review files remain immutable historical evidence and are superseded for current decisions.
...
```text
node scripts/import-bonsai-full.js --dry-run --organization-id <sandbox-org-id> --connections-csv <connection-export.csv> --projects-csv <project-export.csv> --projects-json <bonsai-projects.json> --tasks-csv <task-export.csv> --tasks-json <bonsai-tasks.json> --invoices-csv <invoice-export.csv> --time-entries-csv <time-export.csv> --expenses-csv <expense-export.csv> --addresses-csv <addresses.csv> --summary-file <new-dry-run-report.json>
```

The report is created once with owner-only permissions. Review every source count, mapped owner, create/match/skip count, operating-source registry count, and reconciliation finding. The native all-status project snapshot must match the project CSV one-for-one by immutable ID, title, client, and lifecycle before either can drive a registry record. A missing file, malformed or over-precise financial value, unsupported currency/status, conflicting Hub or registry record, or incomplete financial/date evidence must be resolved at the source or reviewed manually; the importer does not partially parse, guess, move a source identity, or overwrite it.
...
```text
node scripts/import-bonsai-full.js --confirm --organization-id <sandbox-org-id> --connections-csv <connection-export.csv> --projects-csv <project-export.csv> --projects-json <bonsai-projects.json> --tasks-csv <task-export.csv> --tasks-json <bonsai-tasks.json> --invoices-csv <invoice-export.csv> --time-entries-csv <time-export.csv> --expenses-csv <expense-export.csv> --addresses-csv <addresses.csv> --approved-summary <reviewed-dry-run-report.json> --summary-file <new-live-report.json>
```

The confirmed import refuses to start without the reviewed report. It creates each Bonsai project/task source registry row in the same database transaction as its destination and rolls back the whole import if an existing source ID points to another destination or evidence generation. It also rolls back if the sandbox target fingerprint, tenant, source fingerprint, destination plan, required inventory, or reconciliation findings differ from the approved dry run. Existing Hub clients, projects, and invoices are compared and never automatically overwritten. A matching record with different evidence is a manual reconciliation finding. This path is deliberately sandbox-only; production promotion remains a separate post-evidence and explicit cutover gate.

The Bonsai import is only the first side of cross-source consolidation. Every imported Bonsai project, current API task, and native historical task is registered under its immutable source ID in the tenant-scoped operating-source registry before the decision-aware Notion Projects and Tasks plan resolves approved links or proposes Notion-only records. A shared title is never a registry identity. The registry schema, atomic Bonsai registry writes, and fail-closed operating planner are code-present and unapplied; the confirmed Notion operating executor and sandbox proof remain pending.
...
The command is read-only. It verifies the agreed parallel duration, complete operating and financial record scope, zero unresolved discrepancies, separate currency evidence, the full Stripe/email sandbox journey, backups and isolated restore, Cameron's post-evidence approval, the exact 28-artifact inventory, path containment, and every checksum. The retained import inventory includes Connections, project CSV, native all-status project JSON, current task JSON, historical task CSV, invoices, time entries, expenses, and addresses. Both the clean dry run and confirmed report must bind those exact nine source artifacts, the same tenant and sandbox target, the same computed source and plan fingerprints, and zero operating-source registry conflicts. The Notion evidence must contain the verified operating snapshot, exact `READY` plan, completed sandbox result, privacy-bounded inventories from immediately before and after execution, and a zero-finding reconciliation. The plan must bind the snapshot and before inventory; the result must bind the plan and the same sandbox used by the confirmed Bonsai import; and reconciliation must prove that all baseline records were preserved and every addition was planned. Generic or fabricated placeholder Notion files cannot satisfy cutover. The version 3 operating report must bind the exact retained Bonsai Connections, project, invoice, time-entry, and expense sources to an internally valid tenant workspace export version 3, its internal record checksum, and all nine collection counts, including users, time entries, and expenses. The separate version 2 task report must bind the complete all-scope API task snapshot, native historical task CSV, and project CSV to that same exact workspace artifact and all collection counts; listing `tasks` in the manifest is not accepted as evidence by itself. Legacy operations reports remain useful recovery evidence but cannot satisfy cutover readiness. The final revenue artifact must match the manifest's Ashbi organization, pass its internal verifier, and be exported after the parallel run ends but before evidence is finalized. The parallel report must reference the exact retained Bonsai `invoices.csv` checksum, revenue file checksum, internal record checksum, and every collection count; a report copied from another source or export cannot pass. The command reports only generic pass/fail messages and never disables Bonsai.
