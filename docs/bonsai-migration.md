...
2. Compare clients, active projects, statuses, owners, invoice totals by currency, time, and expenses between Bonsai and the Hub.
   ...
   Copy [parallel-reconciliation.example.json](parallel-reconciliation.example.json) into the owner-only evidence directory. Record the revenue artifact file checksum, its internal record checksum, and every collection count exactly; do not copy values from an earlier export. Set `completedAt` only after the comparison is complete and retain any exceptions outside the passing report until they are resolved.
...
The command is read-only. It verifies the agreed parallel duration, complete operating and financial record scope, zero unresolved discrepancies, separate currency evidence, the full Stripe/email sandbox journey, backups and isolated restore, Cameron's post-evidence approval, the required artifact inventory, path containment, and every checksum. The final revenue artifact must match the manifest's Ashbi organization, pass its internal verifier, and be exported after the parallel run ends but before evidence is finalized. The parallel report must then reference that exact file checksum, internal record checksum, and every collection count; a report copied from another export cannot pass. The command reports only generic pass/fail messages and never disables Bonsai.
