...
After every source refresh, regenerate all downstream reviews and decisions before claiming the evidence is current. Then checksum-bind the aligned task review, task mapping decision, task-link decision, owner decision, native project review, project-link decision, active triage, and financial review:

```text
npm run prepare:bonsai-live-reconciliation-status -- --task-review <task-review.json> --mapping-decision <mapping-decision.json> --task-link-decision <task-link-decision.json> --owner-decision <owner-decision.json> --native-project-review <native-project-review.json> --project-link-decision <project-link-decision.json> --active-project-triage <triage.json> --financial-review <financial-review.json> --prepared-at <ISO> --output <new-live-status.json>
npm run verify:bonsai-live-reconciliation-status -- --task-review <task-review.json> --mapping-decision <mapping-decision.json> --task-link-decision <task-link-decision.json> --owner-decision <owner-decision.json> --native-project-review <native-project-review.json> --project-link-decision <project-link-decision.json> --active-project-triage <triage.json> --financial-review <financial-review.json> --status <live-status.json>
```
...
The current 2026-08-28 UTC status is source-generation aligned but not migration- or retirement-ready. It contains 16 blocking findings and has SHA-256 `c70c676c6eb451eff187a5b3977aab9b05ad61d140ea1d6140d59b4b0862805b`. The dedicated task-link record now exists, but all 20 task identities remain pending; title evidence is not approval. Earlier live-status files remain immutable historical evidence and are superseded for current decisions.
