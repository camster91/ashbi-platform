...
After every source refresh, regenerate all downstream reviews and decisions before claiming the evidence is current. Then checksum-bind the aligned task review, task mapping decision, task-link decision, task-disposition decision, owner decision, native project review, project-link decision, project-disposition decision, active triage, and financial review:

```text
npm run prepare:bonsai-live-reconciliation-status -- --task-review <task-review.json> --mapping-decision <mapping-decision.json> --task-link-decision <task-link-decision.json> --task-disposition-decision <task-disposition-decision.json> --owner-decision <owner-decision.json> --native-project-review <native-project-review.json> --project-link-decision <project-link-decision.json> --project-disposition-decision <project-disposition-decision.json> --active-project-triage <triage.json> --financial-review <financial-review.json> --prepared-at <ISO> --output <new-live-status.json>
npm run verify:bonsai-live-reconciliation-status -- --task-review <task-review.json> --mapping-decision <mapping-decision.json> --task-link-decision <task-link-decision.json> --task-disposition-decision <task-disposition-decision.json> --owner-decision <owner-decision.json> --native-project-review <native-project-review.json> --project-link-decision <project-link-decision.json> --project-disposition-decision <project-disposition-decision.json> --active-project-triage <triage.json> --financial-review <financial-review.json> --status <live-status.json>
```
...
The current 2026-08-28 UTC status is source-generation aligned but not migration- or retirement-ready. It contains 15 blocking findings and has SHA-256 `963c32f672ea2884936ff0a7d6238f4830c924918ac62de526aef21ba7500c65`. The dedicated task-link record exists with all 20 task identities pending; the task-disposition record keeps all 48 source-only or malformed task decisions pending; and the project-disposition record keeps all 204 source-only or duplicate-title decisions pending behind the 24 project-link decisions. Title and source evidence are not approval. Earlier live-status files remain immutable historical evidence and are superseded for current decisions.
