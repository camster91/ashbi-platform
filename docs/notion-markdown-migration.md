...
After the Bonsai sandbox baseline has been imported and every Bonsai project and API task has an `OperatingSourceRecord`, export the exact bound sandbox inventory directly from the Hub database:

```text
npm run export:operating-destination-inventory -- --organization-id <sandbox-id> --output <new-sandbox-destination-inventory.json> [--captured-at <ISO>]
```

The read-only exporter fails closed unless the database, application URL, environment ID, sandbox flag, and selected organization identify the configured migration sandbox. It creates one owner-only file, refuses overwrite, removes a partial file after failure, and includes only tenant IDs, project/task identity and lifecycle fields, and operating-source identity evidence. Client names and contact data, descriptions, messages, credentials, financial data, and source content are excluded. The artifact is complete, canonical, relationship-checked, and privacy-attested; the planner rejects extra fields, orphaned or cross-tenant destinations, unresolved imported/linked registry rows, source-only outcomes with destinations, invalid states, duplicates, or a changed safeguard. Create a separate owner-only bindings document mapping every Notion-only project source URL to an existing sandbox client ID and an explicitly reviewed Hub project lifecycle. Do not derive a client or lifecycle from a title. Exact reruns compare imported names, client bindings, lifecycles, titles, task states, and relationships instead of accepting the registry row alone.
...
The planner hashes the bytes of every supplied evidence artifact—including the verified destination inventory—and refuses overwrite. It emits no actions while any identity or disposition is pending or invalid; any approved Notion/Bonsai link must resolve through the exact registered Bonsai destination; a linked task must belong to the resolved project; and a Notion-only project cannot be planned without an existing tenant-owned client plus an explicit supported lifecycle. An exact rerun with unchanged source fingerprints reuses existing destination records and plans zero writes. A `READY` plan is review evidence only and performs no mutation.

After a plan is `READY`, the exact sandbox backup and approval references are configured, and the same evidence remains unchanged, run the confirmed sandbox executor:

```text
npm run execute:notion-operating-migration -- --organization-id <sandbox-id> --notion <notion-projects-tasks.json> --task-review <task-review.json> --task-link-decision <task-link-decision.json> --task-disposition-decision <task-disposition-decision.json> --native-project-review <native-project-review.json> --project-link-decision <project-link-decision.json> --project-disposition-decision <project-disposition-decision.json> --destination-inventory <sandbox-destination-inventory.json> --project-bindings <notion-project-bindings.json> --plan <ready-migration-plan.json> --result <new-migration-result.json> --confirm [--mapping-decision <mapping-decision.json>]
```

The executor re-hashes and exactly recomputes the plan, requires the selected tenant to match the configured migration sandbox, requires backup and approval references, and reserves a new owner-only result before opening the database transaction. Inside one serializable transaction it recaptures the privacy-bounded inventory and refuses every action if any client, project, task, or source identity changed after review. It can create only planned tenant projects and tasks and register their Notion source identities; typed references prevent a task or source from resolving to the wrong entity. It cannot assign owners or change clients, Notion, Bonsai, invoices, payments, contracts, time, expenses, or cutover state. A failed or stale execution is rolled back.

After a completed execution, export a new destination inventory without overwriting the pre-import inventory, then reconcile the exact before/after state:

```text
npm run export:operating-destination-inventory -- --organization-id <sandbox-id> --output <new-after-inventory.json> [--captured-at <ISO>]
npm run reconcile:notion-operating-migration -- --organization-id <sandbox-id> --notion <notion-projects-tasks.json> --before-inventory <before-inventory.json> --after-inventory <after-inventory.json> --plan <ready-plan.json> --result <completed-result.json> --reconciled-at <ISO> --output <new-reconciliation.json>
```

The read-only reconciliation proves that every baseline client, project, task, and source identity is unchanged; every new project, task, and source identity was explicitly planned; created task/project relationships and source destinations are exact; executor counts and safeguards match; and no extra record appeared. It binds the source snapshot, both inventories, plan, execution result, tenant, sandbox target, and evidence timeline by checksum. The Bonsai cutover gate requires a complete zero-finding reconciliation and the same sandbox target used by the confirmed Bonsai import. A completed sandbox execution still requires parallel-run proof, backups, and explicit financial cutover approval before Bonsai retirement.
