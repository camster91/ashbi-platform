...
#
# Run from setup.sh BEFORE `npx wp-env start`. Idempotent.
...
npx --yes wp-env run tests-cli wp plugin activate ashbi-agency-wp-bridge

# Plugin options used for HMAC. Must match hub WP_BRIDGE_SECRET for the
# plugin → hub register/health POSTs to authenticate.
log "writing hub_url + hub_secret into plugin options"
npx --yes wp-env run tests-cli wp option update ashbi_hub_url    "$HUB_URL"
npx --yes wp-env run tests-cli wp option update ashbi_secret_key "$HUB_SECRET"
...
log "plugin ready (constants + dispatch fix verified)"